...
for i in range(22):
    ...
    labeled_data2 = ffdb.prep_trim()
    vehicle_data = ffdb.fetch_vehicle_details(labeled_data2, token)

    if vehicle_data:
        ffdb.update_vehicle_details_in_db(conn, vehicle_data)

    # Step 4: Get valuation data
    labeled_data3 = ffdb.prep_valu()
    valu_data = ffdb.fetch_valu(labeled_data3, token)
